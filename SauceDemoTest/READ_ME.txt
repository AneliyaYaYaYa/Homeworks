...
I have created a various helping classes here.
 ...
 - BaseTest - startUp and terminate the driver before and after each test; Tried the resetApp button but did not work as expected so it is a work in progress. This base class also contains several methods.
...
### TEST/SAUCEDEMOTESTS
 ...
 - ProductTests - I have prepared 2 versions of productAddedToShoppingCart_when_addToCart test - one that checks all items and their prices using CSV tests and another one fo the homewrok itself. You can run whichever you like. :)


## Browser Support

The tests can be configured to run on different browsers by changing the `browserType` variable in the BaseTest/startUp. Supported browsers include:
